...
export async function createPayment(data: CreatePaymentSchema) {
  ...
  if (!data.creatorId) {
    return {
      error: "Creator não foi encontrado."
    }
  }
  ...
  try {
    ...
    if (!creator) {
      return {
        error: "Falha ao criar pagamento, tente mais tarrde"
      }
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      payment_intent_data: {
        application_fee_amount: applicationFeeAmount, //Esee valor precisa estar em centavos
        transfer_data: {
